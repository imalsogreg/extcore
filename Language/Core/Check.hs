...
instance Functor CheckRes where
    fmap f (OkC   c) = OkC (f c)
    fmap _ (FailC s) = FailC s

instance Applicative CheckRes where
    pure = OkC
    x <*> y  = ap x y
...
extendM :: (Ord a, Show a) => Bool -> EnvType -> M.Map a b -> (a,b) -> CheckResult (M.Map a b)
...
extendVenv :: (Ord a, Show a) => Bool -> M.Map a b -> (a,b) -> CheckResult (M.Map a b)
extendVenv check = extendM check NotTv

extendTvenv :: (Ord a, Show a) => M.Map a b -> (a,b) -> CheckResult (M.Map a b)
extendTvenv = extendM True Tv

lookupM :: (Ord a, Show a) => M.Map a b -> a -> CheckResult (Maybe b)
...
mlookupM :: (Eq a, Show a, Show b) => (Envs -> M.Map a b) -> M.Map a b -> M.Map a b -> Mname
          -> CheckResult (M.Map a b)
...
qlookupM :: (Ord a, Show a,Show b) => (Envs -> M.Map a b) -> M.Map a b -> M.Map a b
