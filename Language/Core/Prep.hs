...
prepModule globalEnv (Module mn tdefs vdefgs) =
  ...
  where
    ...
    prepExp env e@(Var qv) | isPrimVar qv = 
     -- trace ("prepExp: e = " ++ show e) $
        unwindApp env e []
    prepExp _ (Var qv) = 
      return $ -- trace ("prepExp: v = " ++ show qv) $
        Var qv
    prepExp env e@(Dcon _) = unwindApp env e []
    ...
    unwindApp env (op@(Var qv)) as | isPrimVar qv = do
        e' <- rewindApp env op as
        out_ <- etaExpand (snd (unzip (drop (length ts) tbs))) (drop n atys) e'
        let (bs,body) = splitLambda out_
        let (tbinds,vbinds) = splitBinds bs
        let out = foldr (\ (t,k) e -> Lam (Tb (t,k)) e)
              (foldr (\ (v,t) e -> Lam (Vb (v,t)) e)
                     body (map (\ (v,t) -> (v,substl (map fst tbs)
                                                     (map (Tvar . fst) tbinds) t))
                            vbinds)) tbinds
        return $
         -- trace ("in = " ++ show op ++ " and " ++ show as ++ ", out = " ++ show out) $
            out
        where (tbs,atys0,_) = splitTy (qlookup venv_ eempty qv)
              atys = map (substl (map fst tbs) ts) atys0
              ts = [t | Right t <- as]
              n = length [e | Left e <- as]
    ...
    mlookup :: (Envs -> M.Map a b) -> M.Map a b -> Mname -> M.Map a b
    ...
    qlookup ::  (Ord a, Show a) => (Envs -> M.Map a b) -> M.Map a b -> (Mname,a) -> b
